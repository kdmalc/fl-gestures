...
architecture_space = {
    "num_conv_layers": [2, 3],
    "conv_layer_sizes": [[32, 64], [32, 64, 128]],
    "kernel_size": [3, 5],
    "stride": [1, 2],
    "padding": [0, 1]
}
...
def run_hyperparameter_tuning(train_dataset, intra_test_loader, cross_test_loader, num_configs_to_test=10, save_path="ELEC573_Proj\\results\\hyperparam_tuning"):
    ...
    results = sorted(results, key=lambda x: x["performance"], reverse=True)

    # Generate unique filename with timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")

    # Save to CSV
    df = pd.DataFrame([{**r["config"], "train_acc": r["train_acc"], "intra_test_acc": r["intra_test_acc"], "cross_test_acc": r["cross_test_acc"]} for r in results])
...
# Example usage
results = run_hyperparameter_tuning(train_dataset, intra_test_loader, cross_test_loader, num_configs_to_test=20)
